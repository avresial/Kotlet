...
{
    private readonly string planStart = planStart.ToString("yyyy-MM-dd");
    ...
    /// </remarks>
    public async Task<IReadOnlyList<(string Label, string Screen, string Path)>> CallsAsync()
    {
        ...
        if (await FirstRecipeIdAsync() is { } recipeId)
    ...
    /// </summary>
    private async Task<string?> FirstRecipeIdAsync()
    {
        var page = await client.GetFromJsonAsync<JsonElement>("/api/recipes?page=1&pageSize=1");
        if (!page.TryGetProperty("items", out var items) || items.GetArrayLength() == 0)
            return null;
