...
export class AgentMemoryPage implements OnInit {
  ...
  beginCreate(): void {
    ...
    this.form.reset({ title: '', content: '', category: '', source: 'ExplicitRequest', reviewStatus: 'Confirmed', confidence: null, expiresAt: '' });
  }
  ...
  beginEdit(memory: AgentMemory): void {
    ...
    this.form.reset({
      ...
      source: memory.source, reviewStatus: memory.reviewStatus, confidence: memory.confidence,
  ...
  save(): void {
    ...
    const draft: AgentMemoryDraft = {
      ...
      source: value.source, reviewStatus: value.reviewStatus, confidence: value.confidence,
  ...
  remove(memory: AgentMemory): void {
    ...
    this.service.delete(memory.id, memory.version).subscribe({
      next: () => this.load(),
  ...
  setReviewStatus(memory: AgentMemory, reviewStatus: string): void {
    ...
    this.service.update(memory.id, draft, memory.version).subscribe({
      next: () => this.load(),
      error: error => this.error.set(getApiError(error, this.translations.translate('memory.saveError'))),
    });
  }
