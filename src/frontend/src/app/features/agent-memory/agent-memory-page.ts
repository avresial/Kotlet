...
export class AgentMemoryPage implements OnInit {
  ...
  load(): void {
    ...
    this.service
      ...
      .subscribe({
        next: (result) => {
          this.memories.set(result.memories);
          this.revision.set(result.revision);
        },
        error: (error) =>
          this.error.set(getApiError(error, this.translations.translate('memory.loadError'))),
      });
  }
  ...
  save(): void {
    ...
    const draft: AgentMemoryDraft = {
      ...
      expiresAt: value.expiresAt ? new Date(`${value.expiresAt}T23:59:59Z`).toISOString() : null,
      clientId: 'web',
    };
    const request = editing
      ? this.service.update(editing.id, draft, editing.version)
      : this.service.create(draft);
    request.pipe(finalize(() => this.saving.set(false))).subscribe({
      next: (result) => {
        if (result.status !== 'Success' || !result.memory) {
          this.error.set(result.message ?? this.translations.translate('memory.saveError'));
          return;
        }
        this.editingId.set(null);
        this.load();
      ...
      error: (error) =>
        this.error.set(getApiError(error, this.translations.translate('memory.saveError'))),
    });
  }
  ...
  remove(memory: AgentMemory): void {
    ...
    this.saving.set(true);
    this.service
      .delete(memory.id, memory.version)
      .pipe(finalize(() => this.saving.set(false)))
      .subscribe({
        next: () => this.load(),
        error: (error) =>
          this.error.set(getApiError(error, this.translations.translate('memory.deleteError'))),
      });
  }
  ...
  setReviewStatus(memory: AgentMemory, reviewStatus: string): void {
    ...
    this.service
      .update(memory.id, draft, memory.version)
      .pipe(finalize(() => this.saving.set(false)))
      .subscribe({
        next: () => this.load(),
        error: (error) =>
  ...
  exportMarkdown(): void {
    this.service.exportMarkdown().subscribe({
      ...
      error: (error) =>
        this.error.set(getApiError(error, this.translations.translate('memory.exportError'))),
    });
  }
}
