...
{
    public async Task<IReadOnlyList<PreparedMealResponse>> ListAsync(
        Guid houseId,
        bool includeArchived,
        CancellationToken ct) =>
        (await repository.ListAsync(houseId, includeArchived, ct)).Select(ToResponse).ToList();

    public async Task<PreparedMealResponse?> GetAsync(
        Guid id,
        Guid houseId,
        CancellationToken ct) =>
        await repository.GetAsync(id, houseId, false, ct) is { } meal ? ToResponse(meal) : null;

    public async Task<PreparedMealOperationResult> CreateAsync(
        Guid houseId,
    ...
    public async Task<PreparedMealOperationResult> UpdateAsync(
        Guid id,
        Guid houseId,
        SavePreparedMealRequest request,
        CancellationToken ct)
    ...
    public async Task<PreparedMealOperationStatus> SetArchivedAsync(
        Guid id,
        Guid houseId,
        bool archived,
        CancellationToken ct)
    ...
    private static void Apply(PreparedMeal meal, SavePreparedMealRequest request)
    {
        ...
        meal.Brand = request.Brand?.Trim();
        meal.Store = request.Store?.Trim();
        meal.Category = request.Category?.Trim();
        ...
        foreach (var addon in request.Addons.OrderBy(a => a.SortOrder))
        {
            meal.Addons.Add(new PreparedMealAddon
            {
                Id = Guid.NewGuid(),
                IngredientId = addon.IngredientId,
                DefaultQuantity = Quantity.FromAmount(addon.Quantity),
                Unit = addon.Unit.Trim(),
                IsSelectedByDefault = addon.IsSelectedByDefault || addon.IsRequired,
    ...
    private static PreparedMealResponse ToResponse(PreparedMeal meal) => new(
        meal.Id,
        meal.Name,
        meal.Description,
        meal.Brand,
        meal.Store,
        meal.Category,
        meal.PackageQuantity,
        meal.PackageUnit,
        meal.Servings,
        meal.CaloriesPerServing,
        meal.Price,
        meal.PreparationInstructions,
        meal.ShoppingIngredientId,
        meal.IsArchived,
        meal.Addons
            .OrderBy(addon => addon.SortOrder)
            .Select(addon => new PreparedMealAddonResponse(
                addon.Id,
                addon.IngredientId,
                addon.Ingredient?.Name ?? "Unknown ingredient",
                addon.DefaultQuantity.Amount,
                addon.Unit,
                addon.IsSelectedByDefault,
                addon.IsRequired,
                addon.SortOrder))
            .ToList());
}
