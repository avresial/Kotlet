...
public static class AgentMemoryEndpoints
{
    ...
    {
        var group = endpoints
            .MapGroup("/api/agent-memory")
            .WithTags("Agent memory")
        ...
        group.MapGet(
            ...
            {
                if (user.UserId is not { } userId || user.HouseId is not { } houseId)
        ...
        group.MapGet(
            "/changes",
            async (long since, ICurrentUser user, AgentMemoryService service, CancellationToken ct) =>
        ...
        group.MapGet(
            "/export",
            async (ICurrentUser user, AgentMemoryService service, CancellationToken ct) =>
                user.UserId is not { } userId || user.HouseId is not { } houseId
                    ? Results.Unauthorized()
        ...
        group.MapGet(
            "/{id:guid}",
            async (Guid id, ICurrentUser user, AgentMemoryService service, CancellationToken ct) =>
                user.UserId is not { } userId || user.HouseId is not { } houseId
                    ? Results.Unauthorized()
        ...
        group.MapPost(
            "",
            async (
                [FromBody] CreateAgentMemoryRequest request,
                ICurrentUser user,
                AgentMemoryService service,
                CancellationToken ct) =>
                user.UserId is not { } userId || user.HouseId is not { } houseId
        ...
        group.MapPut(
            ...
            async (
                Guid id,
                [FromBody] UpdateAgentMemoryRequest request,
                ICurrentUser user,
                AgentMemoryService service,
                CancellationToken ct) =>
                user.UserId is not { } userId || user.HouseId is not { } houseId
