...
public static class FoodSettingsEndpoints
{
    ...
    {
        var group = endpoints
            .MapGroup("/api/users/me/food-settings")
            .WithTags("Food settings")
            .RequireAuthorization();

        group.MapGet(
            "",
            async (ICurrentUser user, UserFoodSettingsService service, CancellationToken ct) =>
                user.UserId is { } id
        ...
        group.MapPut(
            "",
            async (
                SaveUserFoodSettingsCommand command,
                ICurrentUser user,
                UserFoodSettingsService service,
