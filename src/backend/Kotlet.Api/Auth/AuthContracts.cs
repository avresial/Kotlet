...
public sealed record UpdateProfileRequest(string? DisplayName, string? PreferredLanguage);
public sealed record ChangePasswordRequest(string CurrentPassword, string NewPassword, string ConfirmPassword);
public sealed record CurrentUserResponse(Guid Id, string Email, string? DisplayName, string? PreferredLanguage, DateTime CreatedAtUtc, DateTime? LastLoginAtUtc);
