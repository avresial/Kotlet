...
public static class PantryEndpoints
{
    public static IEndpointRouteBuilder MapPantryEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        pantry.MapGet(
            "",
            async (ICurrentUser user, PantryService service, ILanguageContext language, CancellationToken ct) =>
        ...
        pantry.MapGet(
            ...
            async (
                ICurrentUser user,
                PantryRecipeMatchService service,
                ILanguageContext language,
                CancellationToken ct) =>
                user.HouseId is { } houseId
                    ? Results.Ok(await service.GetSuggestionsAsync(houseId, language.Language, ct))
                    : Results.Unauthorized())
            .WithName("GetPantryRecipeMatches");

        pantry.MapPost("", Create)
            .WithName("CreatePantryItem");

        pantry.MapPut("/{id:guid}", Update)
            .WithName("UpdatePantryItem");

        pantry.MapDelete("/{id:guid}", Delete)
    ...
    private static async Task<IResult> Update(
        Guid id,
        UpdatePantryQuantityCommand command,
        ICurrentUser user,
        PantryService service,
        ILanguageContext language,
        CancellationToken ct) =>
        user.HouseId is { } houseId
            ? ToHttpResult(await service.UpdateAsync(id, houseId, command.Quantity, language.Language, ct), false)
            : Results.Unauthorized();
    ...
    private static async Task<IResult> Delete(
        ...
        user.HouseId is not { } houseId
            ? Results.Unauthorized()
            : await service.DeleteAsync(id, houseId, ct) == PantryOperationStatus.Success
                ? Results.NoContent()
                : Results.NotFound();
