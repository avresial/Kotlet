...
public static class PantryEndpoints
{
    public static IEndpointRouteBuilder MapPantryEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        pantry.MapGet("", async (ICurrentUser user, PantryService service, CancellationToken ct) =>
            user.HouseId is { } houseId ? Results.Ok(await service.GetAllAsync(houseId, ct)) : Results.Unauthorized()).WithName("GetPantry");
    ...
    private static async Task<IResult> Create(SavePantryItemCommand command, ICurrentUser user, PantryService service, CancellationToken ct) =>
        user.HouseId is { } houseId ? ToHttpResult(await service.CreateAsync(houseId, command, ct), true) : Results.Unauthorized();
    private static async Task<IResult> Update(Guid id, UpdatePantryQuantityCommand command, ICurrentUser user, PantryService service, CancellationToken ct) =>
        user.HouseId is { } houseId ? ToHttpResult(await service.UpdateAsync(id, houseId, command.Quantity, ct), false) : Results.Unauthorized();
