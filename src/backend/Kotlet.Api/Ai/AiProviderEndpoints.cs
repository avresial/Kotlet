...
public static class AiProviderEndpoints
{
    ...
    {
        var group = endpoints
            .MapGroup("/api/ai-provider")
            .WithTags("AI provider")
            .RequireAuthorization();

        group.MapGet(
            "",
            async (ICurrentUser user, UserAiProviderService service, CancellationToken ct) =>
                user.UserId is not { } userId
                    ? Results.Unauthorized()
                    : await service.GetAsync(userId, ct) is { } configuration
    ...
    private static async Task<IResult> Save(
        SaveAiProviderConfigurationCommand command,
        ICurrentUser user,
