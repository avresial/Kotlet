...
public static class PreparedMealEndpoints
{
    ...
    private static async Task<IResult> List(
        ...
        user.HouseId is { } houseId
            ? Results.Ok(await service.ListAsync(houseId, includeArchived, ct))
    ...
    private static async Task<IResult> Get(
        ...
        user.HouseId is not { } houseId
            ? Results.Unauthorized()
            : await service.GetAsync(id, houseId, ct) is { } meal
                ? Results.Ok(meal)
                : Results.NotFound();
    ...
    private static async Task<IResult> Create(
        ...
        user.HouseId is not { } houseId
            ? Results.Unauthorized()
            : ToResult(await service.CreateAsync(houseId, request, ct), true);
    ...
    private static async Task<IResult> Update(
        ...
        user.HouseId is not { } houseId
            ? Results.Unauthorized()
            : ToResult(await service.UpdateAsync(id, houseId, request, ct), false);
    ...
    private static async Task<IResult> Archive(
        ...
        CancellationToken ct) =>
        user.HouseId is { } houseId
        && await service.SetArchivedAsync(id, houseId, true, ct) == PreparedMealOperationStatus.Success
            ? Results.NoContent()
            : Results.NotFound();
    ...
    private static async Task<IResult> Restore(
        ...
        CancellationToken ct) =>
        user.HouseId is { } houseId
        && await service.SetArchivedAsync(id, houseId, false, ct) == PreparedMealOperationStatus.Success
            ? Results.NoContent()
            : Results.NotFound();

    private static IResult ToResult(PreparedMealOperationResult result, bool created) => result.Status switch
    {
        PreparedMealOperationStatus.Success when created =>
            Results.Created($"/api/prepared-meals/{result.Meal!.Id}", result.Meal),
        PreparedMealOperationStatus.Success => Results.Ok(result.Meal),
        PreparedMealOperationStatus.NotFound => Results.NotFound(),
    ...
    private static async Task<IResult> UploadImage(
        ...
        ICurrentUser user,
        PreparedMealImageService service,
        CancellationToken ct)
    {
        ...
        if (file.Length > PreparedMealImageService.MaxFileSizeBytes)
        {
            ...
            {
                ["file"] = ["Image file cannot exceed 5 MB."]
            });
        }
        ...
        {
            PreparedMealOperationStatus.Success => Results.Created(result.Image!.ContentUrl, result.Image),
            PreparedMealOperationStatus.NotFound => Results.NotFound(),
    ...
    private static async Task<IResult> ListImages(
        ...
        user.HouseId is not { } houseId
            ? Results.Unauthorized()
            : await service.ListAsync(id, houseId, ct) is { } images
                ? Results.Ok(images)
                : Results.NotFound();

    private static async Task<IResult> GetImageContent(
        Guid id,
        Guid imageId,
        ICurrentUser user,
        PreparedMealImageService service,
    ...
    private static async Task<IResult> UpdateImage(
        Guid id,
        Guid imageId,
        UpdatePreparedMealImageRequest request,
        ICurrentUser user,
        PreparedMealImageService service,
        CancellationToken ct) =>
        user.HouseId is not { } houseId
            ? Results.Unauthorized()
            : await service.UpdateAsync(id, imageId, houseId, request.AltText, ct) switch
            {
                PreparedMealOperationStatus.Success => Results.NoContent(),
                PreparedMealOperationStatus.NotFound => Results.NotFound(),
                _ => Results.ValidationProblem(new Dictionary<string, string[]>
                {
                    ["altText"] = ["Alt text cannot exceed 300 characters."]
                })
            };

    private static async Task<IResult> ReorderImages(
        Guid id,
        ReorderPreparedMealImagesRequest request,
        ICurrentUser user,
        PreparedMealImageService service,
        CancellationToken ct) =>
        user.HouseId is not { } houseId
            ? Results.Unauthorized()
            : await service.ReorderAsync(id, houseId, request.ImageIds, ct) switch
            {
                PreparedMealOperationStatus.Success => Results.NoContent(),
                PreparedMealOperationStatus.NotFound => Results.NotFound(),
                _ => Results.ValidationProblem(new Dictionary<string, string[]>
                {
                    ["imageIds"] = ["Image ids must contain every image exactly once."]
                })
            };
    ...
    private static async Task<IResult> DeleteImage(
        ...
        CancellationToken ct) =>
        user.HouseId is { } houseId
        && await service.DeleteAsync(id, imageId, houseId, ct) == PreparedMealOperationStatus.Success
            ? Results.NoContent()
            : Results.NotFound();
}
