...
internal sealed class RecipeImageSourceConfiguration : IEntityTypeConfiguration<RecipeImageSource>
{
    ...
    {
        builder.ToTable("recipe_image_sources");
        builder.HasKey(x => new { x.RecipeImageId, x.SourceId });
        builder.Property(x => x.RecipeImageId).HasColumnName("recipe_image_id");
        ...
        builder.HasOne(x => x.RecipeImage).WithMany(i => i.Sources).HasForeignKey(x => x.RecipeImageId).OnDelete(DeleteBehavior.Cascade);
