---
name: update-recipe
description: Update or correct an existing Kotlet recipe, including adding a source or video link, changing instructions, servings, meal type, or ingredients. Use when asked to edit, update, correct, append to, or change a recipe already stored in Kotlet.
---
...
3. **Apply only the requested change.** Preserve the title, description, servings, meal type,
   source URL, and every ingredient that should remain unchanged.
...
5. **Confirm ambiguous or subtractive changes.** Show the exact replacement when the request could
   remove or replace recipe data. A clear additive request, such as appending a supplied video link,
...
- Preserve the existing source URL when adding a second source or video link; append the additional
  link to `descriptionMarkdown` unless the user explicitly asks to replace the source URL.
